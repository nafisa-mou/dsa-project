...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  void _init() {
    _firebaseAuth.authStateChanges().listen((firebase_auth.User? firebaseUser) async {
      if (firebaseUser == null) {
        ...
        try {
          final doc = await _firestore.collection('students').doc(firebaseUser.uid).get();
          if (doc.exists && doc.data() != null) {
            final data = doc.data()!;
            // Fetch name and email from student record or fallback to firebaseAuth user
            final user = User(
              id: firebaseUser.uid,
  ...
  Future<bool> register({
    ...
    try {
      ...
      // Register student profile in Firestore
      ...
      await _firestore.collection('students').doc(uid).set(studentProfile);
